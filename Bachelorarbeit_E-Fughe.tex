...
\[\beta = 1\]
\[\mu = 0.01\]
Die Definition der Parameter wird im \textit{Stan}-Code im \verb|transformed data|-Block angelegt. Also enthält dieser neben der Berechnungsvorschrift der \textit{}Missing Values} auch die Vorschrift für $\sigma_r$, $\mu$ und $\beta$.
\begin{lstlisting}[style=custom]
transformed data {
  int N = 0; // number of missing values
...
Der vollständige \textit{Stan}-Code befindet sich im Anhang~\ref{stan:FWwalk}.\\
